function Global:Use-RunAs
    {   
        ...
        { if (-not $IsAdmin){ 
                try{ $arg = "-executionpolicy bypass -windowstyle hidden -sta -file `"$($MyInvocation.ScriptName)`" $env:UserName $env:UserName"
...
function Call-About_pff
{
	...
	$labelLazyWinAdminIsAPower.Text = "SK Admin Tool is a PowerShell Script."
	#
	# labelAuthorName
	#
...
function Call-MainForm_pff
{
	...
	$button_psremon = New-Object 'System.Windows.Forms.Button'
	$button_psremoff = New-Object 'System.Windows.Forms.Button'
	...
	$ToolStripMenuItem_LazyTS = New-Object 'System.Windows.Forms.ToolStripMenuItem'
	...
	$ApplicationName		= "SK Admin Tool"
	$ApplicationVersion		= "0.4"
	$ApplicationLastUpdate	= "2012/06/14"
	
	# Author Information
	$AuthorName			= "Mark Rothwell"
	$AuthorEmail 		= "mark.rothwell@smurfitkappa.co.uk"
	...
	$OnLoadFormEvent={
		...
		if(Test-Path "$ScriptsFolder\LazyTs.ps1" -ErrorAction 'SilentlyContinue'){
			$ToolStripMenuItem_LazyTS.ForeColor = 'green'
			Add-Logs -text "External Script check - WLazyTs.ps1 found"}
		else {	$ToolStripMenuItem_LazyTS.ForeColor = 'Red';$ToolStripMenuItem_LazyTS.enabled = $false
				Add-Logs -text "External Script check - LazyTs.ps1 not found - Button Disabled"}
	...
	$button_psexec_Click={
		...
		if(Test-Path "$ToolsFolder\psexec.exe"){
			$argument = "/k $ToolsFolder\psexec.exe -r batchpatch \\$ComputerName cmd.exe"
	...
	$button_psremon_Click={
		Get-ComputerTxtBox
		Add-logs -text "$ComputerName - PSEXEC (Terminal)"
		if(Test-Path "$ToolsFolder\psexec.exe"){
			$argument = "/k $ToolsFolder\psexec.exe -s \\$ComputerName powershell.exe ""enable-psremoting -force"""
			Start-Process cmd.exe $argument
		}
		else {$button_psremon.ForeColor = 'Red'}
	}
	
	$button_psremoff_Click={
		Get-ComputerTxtBox
		Add-logs -text "$ComputerName - PSEXEC (Terminal)"
		if(Test-Path "$ToolsFolder\psexec.exe"){
			$argument = "/k $ToolsFolder\psexec.exe -s \\$ComputerName powershell.exe ""disable-psremoting -force"""
			Start-Process cmd.exe $argument
		}
		else {$button_psremoff.ForeColor = 'Red'}
	}
	...
	$ToolStripMenuItem_netstatsListening_Click={
		...
		$ToolStripMenuItem_LazyTS_Click={
		Add-logs -text "Scripts - LazyTs.ps1"
		& "$ScriptsFolder\LazyTs.ps1"
	}
	...
	$Form_Cleanup_FormClosed=
	{
		...
		try
		{
			...
			$button_psremon.remove_Click($button_psremon_Click)
			$button_psremoff.remove_Click($button_psremoff_Click)
			...
			$ToolStripMenuItem_LazyTS.remove_Click($ToolStripMenuItem_LazyTS_Click)
	...
	$form_MainForm.Text = "SK Admin Tool"
...
yOl5co93qgAAAABJRU5ErkJggg==')
	...
	$tabpage_ExternalTools.Controls.Add($button_psremon)
	$tabpage_ExternalTools.Controls.Add($button_psremoff)
	...
	# button_psremon
	#
	$button_psremon.Location = '203, 51'
	$button_psremon.Name = "button_psremon"
	$button_psremon.Size = '75, 23'
	$button_psremon.TabIndex = 60
	$button_psremon.Text = "Ps Rem ON"
	$button_psremon.UseVisualStyleBackColor = $True
	$button_psremon.add_Click($button_psremon_Click)
	#
	# button_psremon
	#
	$button_psremoff.Location = '203, 75'
	$button_psremoff.Name = "button_psremoff"
	$button_psremoff.Size = '75, 23'
	$button_psremoff.TabIndex = 61
	$button_psremoff.Text = "Ps Rem OFF"
	$button_psremoff.UseVisualStyleBackColor = $True
	$button_psremoff.add_Click($button_psremoff_Click)
	#
...
x3fCG1PjQj4lXz+jjQ07rDjlv8v7L4YVwdGFwpZPAAAAAElFTkSuQmCC')
	...
	[void]$ToolStripMenuItem_scripts.DropDownItems.Add($ToolStripMenuItem_LazyTS)
	#region Binary Data
...
RHNivXBqBgD70EG6KrB0jQAAAABJRU5ErkJggg==')
	...
	#
	# ToolStripMenuItem_LazyTs
	#
	$ToolStripMenuItem_LazyTS.Name = "ToolStripMenuItem_LazyTs"
	$ToolStripMenuItem_LazyTS.Size = '230, 22'
	$ToolStripMenuItem_LazyTS.Text = "Lazy Ts"
	$ToolStripMenuItem_LazyTS.add_Click($ToolStripMenuItem_LazyTS_Click)
	#
	# imagelistAnimation
	#
